...
export class FrameUpdateSender {
  ...
  public sendFrameUpdate(
    currentFrame: SimulationFrameData,
    componentsAndSerializedData: ComponentAndSerializedData[]
  ): void {
    this.sendFrameInputsUsedMessage(currentFrame);
    this.sendFrameComponentStateMessage(
      currentFrame,
      componentsAndSerializedData
    );
  ...
  private sendFrameInputsUsedMessage(currentFrame: SimulationFrameData) {
    ...
    if (maybeInputs !== undefined) {
      inputsUsedMessage.inputUsedForPlayerThisFrame = new Uint8Array(
        measureAndSerialize(maybeInputs)
      );
    } else {
  ...
  private sendFrameComponentStateMessage(
    currentFrame: SimulationFrameData,
    componentsAndSerializedData: ComponentAndSerializedData[]
  ) {
    let componentStateMessage = new S2C_FrameComponentStateMessage();
    componentStateMessage.simulationFrameIndex =
      currentFrame.simulationFrameIndex;
    ...
    let maxBytesComponentData = Math.round(
      this.networkPeer.getMtuForMessage() / 2
    );
    let componentsToSend = this.chooser.getComponentsToSend(
      componentsAndSerializedData,
      maxBytesComponentData
    );
    let componentBufferLen = componentsToSend.reduce(
      (acc, next) =>
        acc + SIZE_OF_COMPONENT_ID + next.serializedData.byteLength,
      0
    );
    ...
    let outboundMessage = this.networkPeer.sendMessage(
  ...
  private sendDeletionsMessage(currentFrame: SimulationFrameData) {
    ...
    for (let component of currentFrame.state
      .getEntityComponentDb()
      .getAllComponents()) {
      if (component.isDeleted) {
        let maybeDeletedTag = currentFrame.state.getComponent(
          DeletedTag,
          component.id.ownerId
        );
      ...
      if (component.data.constructor === DeletedTag) {
