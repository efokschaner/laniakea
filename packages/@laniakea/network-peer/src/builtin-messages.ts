import { ClassRegistry, Serializable, SerializationStream, ShortTypeId, TypeId } from '@laniakea/utils';
...
export function registerBuiltinMessages(classRegistry: ClassRegistry) {
  ...
  let handshakeTypeInfo = classRegistry.registerClass(S2C_BuiltinHandshakeMessage, '__S2C_BuiltinHandshakeMessage');
  if (handshakeTypeInfo.shortTypeId !== RESERVED_SHORT_ID) {
    // Swap the current occupant with S2C_BuiltinHandshakeMessage
    let currentTypeWithReservedId = classRegistry.getTypeInfoByShortTypeId(RESERVED_SHORT_ID);
    let currentMappings = classRegistry.getTypeIdToShortTypeIdMapping();
    let newMappings = currentMappings.filter(([, shortTypeId]) => !(shortTypeId === RESERVED_SHORT_ID || shortTypeId === handshakeTypeInfo.shortTypeId));
    newMappings.push([handshakeTypeInfo.typeId, RESERVED_SHORT_ID]);
    if (currentTypeWithReservedId !== undefined) {
      newMappings.push([currentTypeWithReservedId.typeId, handshakeTypeInfo.shortTypeId]);
    }
