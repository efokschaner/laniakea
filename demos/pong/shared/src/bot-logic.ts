...
export function getPersistentIndexToWallDataMap(
  ...
  let vertsToConsiderSorted = vertsToConsider.sort(
    (a, b) => a[0].getData().visualIndex - b[0].getData().visualIndex
  );
 ...
 */
export function calculateBestMoveIntent(
  paddle: Paddle,
  wallData: WallData,
  balls: ReadonlyArray<[lk.Component<BallMovement>, lk.Component<Position2>]>
): MoveIntent {
  ...
  let highestBallPriority = Infinity; // Lower priority means more important
  for (let ballComponents of balls) {
    ...
    let closestPointOnWallLine2D = new THREE.Vector2(
      closestPointOnWallLine.x,
      closestPointOnWallLine.y
    );
    let ballToWall = closestPointOnWallLine2D.sub(ballPosition);
    let distanceToWall = ballToWall.length();
    let velocityInOurDirection =
      ballVelocity.dot(ballToWall) / ballToWall.length();
    ...
    if (!isMovingTowardsWall) {
      ...
      let velocityInOurDirectionForScoringPurposes =
        ballVelocity.length() * 0.1;
      let priorityOfMovingAwayBall =
        distanceToWall / velocityInOurDirectionForScoringPurposes;
      if (highestBallPriority > priorityOfMovingAwayBall) {
        highestBallPriority = priorityOfMovingAwayBall;
        desiredWallPosInWallSpace = wallLine.closestPointToPointParameter(
          closestPointOnWallLine,
          true
        );
      }
    ...
    if (highestBallPriority > priority) {
      ...
      ballRay.distanceSqToSegment(
        wallLine.start,
        wallLine.end,
        intersectOfWallWithBall
      );
      desiredWallPosInWallSpace = wallLine.closestPointToPointParameter(
        intersectOfWallWithBall,
        true
  ...
  if (paddle.velocityInWallSpace !== 0) {
    let paddleAccelerationIfWeStopMoving =
      -Math.sign(paddle.velocityInWallSpace) * Paddle.maxAcceleration;
    let timeToRest =
      -paddle.velocityInWallSpace / paddleAccelerationIfWeStopMoving;
    predictedPositionOfRest +=
      paddle.velocityInWallSpace * timeToRest +
      0.5 * paddleAccelerationIfWeStopMoving * timeToRest ** 2;
  }
  let deadZoneProportion = Paddle.lengthAsProportionOfWallLength / 4;
  if (
    predictedPositionOfRest >
    (1 + deadZoneProportion) * desiredWallPosInWallSpace
  ) {
