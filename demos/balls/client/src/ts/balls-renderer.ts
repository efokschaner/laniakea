...
export class RenderingSystemImpl implements lk.RenderingSystem {
  ...
  private rendererSpheres = new Map<lk.EntityId, THREE.Mesh>();
  private rendererWalls = new Map<lk.EntityId, THREE.Mesh>();

  private focusObject(object: THREE.Object3D) {
    let originalTargetPos = this.cameraController.target.clone();
    let targetPosToCameraPos = this.cameraController.object.position
      .clone()
      .sub(originalTargetPos);
    ...
    this.activeCameraLerp = (currentWallTimeMS: number) => {
      ...
      this.cameraController.target = originalTargetPos
        .clone()
        .lerp(targetPosition, lerpFactor);
      this.cameraController.object.position.copy(
        this.cameraController.target.clone().add(targetPosToCameraPos)
      );
    };
  }
  ...
  private getIntersects(
    camera: THREE.Camera,
    topLeftScreenCoord: { x: number; y: number },
    objects: THREE.Object3D[]
  ) {
    let centeredScreenCoord = {
      x: topLeftScreenCoord.x * 2 - 1,
      y: 1 - topLeftScreenCoord.y * 2,
    };
    this.raycaster.setFromCamera(centeredScreenCoord, camera);
    return this.raycaster.intersectObjects(objects);
  }

  public constructor(private sceneElementContainer: HTMLElement) {
  ...
  public render(
    ...
    clientSimulation: lk.ClientSimulation
  ): void {
    this.rendererSizeUpdater.update();
    if (this.activeCameraLerp) {
      this.activeCameraLerp(domHighResTimestampMS);
    }
    ...
    let nearestFrames = clientSimulation.getSimulationFrames(
      simTimeS + inputTravelTimeS
    );
